...
if (
  ...
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "One or more R2 environment variables are not set. File uploads will likely fail.",
    );
  } else {
    ...
    console.error(
      "CRITICAL: R2 environment variables are not fully configured.",
    );
  }
}
...
export async function uploadFileToR2(
  ...
  try {
    const creds =
      typeof s3Client.config.credentials === "function"
    ...
    if (!creds || !creds.accessKeyId || !creds.secretAccessKey) {
      if (R2_ACCESS_KEY_ID && R2_SECRET_ACCESS_KEY) {
        return {
          error:
            "R2 client credentials not properly initialized despite ENV VARS being present.",
        };
      }
  ...
  try {
    ...
    if (!saneBasePath) {
      return {
        error: "Base path for R2 upload cannot be empty or just slashes.",
      };
    }
...
export async function deleteFileFromR2(
  ...
  if (!fileUrl.startsWith(R2_PUBLIC_URL_PREFIX)) {
    console.warn(
      `File URL '${fileUrl}' does not match R2 public prefix. Skipping deletion.`,
    );
