// src/app/admin/lessons/LessonsList.tsx
'use client';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const { data, error: fetchError } = await getAllLessons(courseId ? parseInt(courseId) : undefined);
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {lessons.map((lesson) => (
                                <tr key={lesson.id} className="hover:bg-gray-50">
                                    ...
                                    {selectedCourse === 'all' && (
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-gray-500">{lesson.courseTitle || 'N/A'}</div>
                                        </td>
                                    )}
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-500">{lesson.day_number ?? '-'}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="text-sm text-gray-500">{lesson.createdAt ? new Date(lesson.createdAt).toLocaleDateString() : '-'}</div>
                                    </td>
