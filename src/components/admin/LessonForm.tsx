...
export function LessonForm({
  ...
  useEffect(() => {
    if (initialData?.thumbnail_url && !thumbnailPreviewUrl && !removeThumbnailFlag) {
    ...
    if (!initialData?.thumbnail_url && !removeThumbnailFlag && !document.querySelector<HTMLInputElement>('#thumbnailFile')?.files?.[0]) {
    ...
    <form action={formAction} className="space-y-6 bg-white p-8 shadow-md rounded-lg">
      <div>
        ...
        <select
          id="course_id"
          name="course_id"
          defaultValue={initialData?.course_id || initialValues?.course_id || ''}
          required
        ...
        {state.errors?.course_id && (
          <p className="mt-1 text-xs text-red-500">{state.errors.course_id.join(', ')}</p>
        )}
      </div>
      ...
      <div>
        ...
        {state.errors?.video_url && (
          <p id="video_url-error" className="text-sm font-medium text-destructive">
      ...
      <div>
        ...
        <Input
          ...
          type="number"
          placeholder="1"
          defaultValue={initialData?.day_number?.toString() || initialValues?.day_number?.toString() || ''}
          aria-describedby="day_number-error"
          min="1"
          required
        />
        <p className="text-sm text-muted-foreground">
          The day number this lesson should become available (e.g., 1 for Day 1).
        </p>
        {state.errors?.day_number && (
          <p id="day_number-error" className="text-sm font-medium text-destructive">
      ...
      <div>
        ...
        <textarea
          id="description"
          name="description"
          defaultValue={initialData?.description || initialValues?.description || ''}
          rows={3}
          className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
        />
      ...
      <div>
        ...
        />
        <input type="hidden" name="removeThumbnail" value={removeThumbnailFlag ? "true" : "false"} />

        {thumbnailPreviewUrl && !removeThumbnailFlag && (
          <div className="mt-4">
            <img src={thumbnailPreviewUrl} alt="Thumbnail preview" className="h-40 w-auto object-cover rounded-md border border-gray-200" />
          </div>
        )}

        {(thumbnailPreviewUrl || initialData?.thumbnail_url) && !removeThumbnailFlag && (
          <button 
            type="button" 
        ...
        {removeThumbnailFlag && (
          <p className="mt-2 text-sm text-gray-500">Thumbnail will be removed on save.</p>
        )}
        {state.errors?.thumbnailFile && (
          <p className="mt-1 text-xs text-red-500">{state.errors.thumbnailFile.join(', ')}</p>
        )}
