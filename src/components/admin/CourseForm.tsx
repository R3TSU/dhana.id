...
import type { courses } from '@/db/schema'; // Import the type
...
export function CourseForm({
  ...
  useEffect(() => {
    ...
    if (initialData && initialData.thumbnail_url === null && previewUrl !== null) {
        setPreviewUrl(null);
    }
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setRemoveThumbnailFlag(false); // If a new file is selected, don't remove
    } else {
      // If file selection is cancelled, revert to initial or clear if no initial
      setPreviewUrl(initialData?.thumbnail_url || null);
    }
  };
  ...
  return (
    <form action={formAction} className="space-y-6 bg-white p-8 shadow-md rounded-lg">
      <div>
      ...
      <div>
        ...
        {state.errors?.subtitle && (
          <p className="mt-1 text-xs text-red-500">{state.errors.subtitle.join(', ')}</p>
        )}
      </div>
      ...
      <div className="flex space-x-6">
        <div className="flex items-center">
          ...
          {state.errors?.is_active && (
            <p className="mt-1 text-xs text-red-500">{state.errors.is_active.join(', ')}</p>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
          />
          <p className="mt-1 text-xs text-gray-500">If set, course will only be available after this date</p>
          {state.errors?.start_date && (
            <p className="mt-1 text-xs text-red-500">{state.errors.start_date.join(', ')}</p>
          )}
        </div>
      </div>
      ...
      <div>
        ...
        {previewUrl && (
          <div className="mt-4">
            <img src={previewUrl} alt="Thumbnail preview" className="h-32 w-auto object-cover rounded-md border border-gray-300" />
            <button 
              type="button" 
        ...
        {!previewUrl && initialData?.thumbnail_url && !removeThumbnailFlag && (
            <div className="mt-4">
                ...
                <img src={initialData.thumbnail_url} alt="Current thumbnail" className="h-32 w-auto object-cover rounded-md border border-gray-300" />
                 <button 
                    type="button" 
                    onClick={handleRemoveThumbnail}
                    className="mt-2 text-xs text-red-600 hover:text-red-800"
        ...
        {state.errors?.thumbnail_url && (
          <p className="mt-1 text-xs text-red-500">{state.errors.thumbnail_url.join(', ')}</p>
        )}
         {/* Display errors for _form if they are specific to file upload process */}
        {state.errors?._form && state.errors._form.some((err: string) => err.toLowerCase().includes('upload') || err.toLowerCase().includes('file')) && (
          <p className="mt-1 text-xs text-red-500">{state.errors._form.find((err: string) => err.toLowerCase().includes('upload') || err.toLowerCase().includes('file'))}</p>
        )}
      </div>

      {state.errors?._form && (
        <p className="mt-1 text-sm text-red-600 bg-red-100 p-3 rounded-md">{state.errors._form.join(', ')}</p>
      )}
      {state.message === 'Database Error' && !state.errors?._form && (
         <p className="mt-1 text-sm text-red-600 bg-red-100 p-3 rounded-md">An unexpected database error occurred.</p>
      )}

      <div className="flex items-center justify-end space-x-4">
        <Link href="/admin/courses" className="text-gray-600 hover:text-gray-800 text-sm font-medium">
            Cancel
        </Link>
