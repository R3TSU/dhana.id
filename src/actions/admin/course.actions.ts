...
const courseDbSchema = z.object({
  ...
  slug: z
    .string()
    .min(1, "Slug cannot be empty after generation")
    .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Generated slug is invalid"),
  subtitle: z.string().optional(),
  description: z.string().optional(),
  thumbnail_url: z.string().url("Invalid URL format").nullable().optional(),
...
export async function createCourse(prevState: any, formData: FormData) {
  ...
  if (!validatedDbFields.success) {
    ...
    console.error(
      "Internal validation error for DB data (createCourse):",
      validatedDbFields.error.flatten().fieldErrors,
    );
    return {
      message: "Internal Server Error",
      errors: {
        _form: ["Failed to prepare data for database. Invalid generated slug."],
      },
    };
  }
  ...
  try {
    ...
    if (error.message?.includes("unique constraint")) {
      if (
        error.message?.includes("courses_slug_idx") ||
      ...
      if (
        error.message?.includes("courses_title_idx") ||
        error.message?.includes("courses_title_key")
      ) {
        return {
          message: "Database Error",
    ...
    return {
      message: "Database Error",
      errors: { _form: ["Failed to create course. Please try again."] },
    };
  }
...
export async function updateCourse(
  ...
  if (existingCourseResult.error || !existingCourseResult.data) {
    return {
      message: "Error",
      errors: { _form: ["Course not found or failed to fetch existing data."] },
    };
  }
  ...
  if (!validatedDbFields.success) {
    // This would indicate an issue with slug generation or internal logic
    console.error(
      "Internal validation error for DB data (updateCourse):",
      validatedDbFields.error.flatten().fieldErrors,
    );
    return {
      message: "Internal Server Error",
  ...
  try {
    ...
    if (error.message?.includes("unique constraint")) {
      if (
        ...
        return {
          message: "Database Error",
          errors: { slug: ["Slug already exists for another course."] },
        };
      }
      if (
        error.message?.includes("courses_title_idx") ||
        ...
        return {
          message: "Database Error",
          errors: { title: ["Title already exists for another course."] },
        };
      }
    }
    return {
      message: "Database Error",
      errors: { _form: ["Failed to update course. Please try again."] },
    };
  }
...
export async function getCourseWithLessonsBySlug(courseSlug: string): Promise<{
  ...
  try {
    ...
    const processedLessons: PublicLessonForCoursePage[] = lessonsData.map(
      (lesson) => ({
        ...lesson,
