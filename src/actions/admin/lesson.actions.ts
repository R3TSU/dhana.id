...
const lessonDbSchema = lessonFormSchema.extend({
  slug: z.string().min(1, 'Slug cannot be empty').regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'Slug must be lowercase and hyphenated'),
});
...
export async function getLessonById(id: number) {
  ...
  try {
    const data = await db.select().from(lessons).where(eq(lessons.id, id)).limit(1);
    if (data.length === 0) {
...
export async function createLesson(prevState: any, formData: FormData) {
  ...
  try {
    ...
    if (thumbnailFile && thumbnailFile.size > 0) {
      const uploadResult = await uploadFileToR2(thumbnailFile, 'lessons/thumbnails');
      if ('error' in uploadResult) {
        return { message: 'Upload Error', errors: { thumbnailFile: [uploadResult.error] } };
      }
    ...
    return { message: 'Database Error', errors: { _form: ['Failed to create lesson. Please try again.'] } };
  }
...
export async function updateLesson(id: number, prevState: any, formData: FormData) {
  ...
  try {
    ...
    if (removeThumbnail) {
      if (currentThumbnailUrl) {
        ...
        if ('error' in deleteResult) {
          ...
          console.warn(`Failed to delete old thumbnail from R2: ${deleteResult.error}`);
        }
      }
      ...
      if (currentThumbnailUrl) {
        const deleteResult = await deleteFileFromR2(currentThumbnailUrl);
        if ('error' in deleteResult) {
          console.warn(`Failed to delete old thumbnail from R2 during replacement: ${deleteResult.error}`);
        }
      }
      const uploadResult = await uploadFileToR2(thumbnailFile, 'lessons/thumbnails');
      if ('error' in uploadResult) {
      ...
      dataToUpdate.thumbnail_url = validatedDbFields.data.thumbnail_url || currentThumbnailUrl || '';
    }

    // Remove helper fields not in DB schema before update
    const { thumbnailFile: _tf, removeThumbnail: _rtf, ...dbDataForUpdate } = dataToUpdate;
    ...
    console.error('Error updating lesson:', error);
     if (error.message?.includes('unique constraint')) {
        if (error.message?.includes('lessons_slug_idx') || error.message?.includes('lessons_slug_key')) {
            return { message: 'Database Error', errors: { slug: ['Slug already exists for another lesson.'] } };
        }
        if (error.message?.includes('lessons_title_idx') || error.message?.includes('lessons_title_key')) {
            return { message: 'Database Error', errors: { title: ['Title already exists for another lesson.'] } };
        }
    }
    return { message: 'Database Error', errors: { _form: ['Failed to update lesson. Please try again.'] } };
  }
...
export async function deleteLesson(id: number, course_id?: number) {
  ...
  try {
    const lessonData = await db.select({ thumbnail_url: lessons.thumbnail_url, course_id: lessons.course_id }).from(lessons).where(eq(lessons.id, id)).limit(1);
    ...
    if (thumbnail_url) {
      ...
      if ('error' in deleteResult) {
        ...
        console.warn(`Failed to delete thumbnail from R2 for lesson ${id}: ${deleteResult.error}`);
      }
    }
...
export async function getCoursesForSelection() {
    try {
        const data = await db.select({ id: courses.id, title: courses.title }).from(courses).orderBy(asc(courses.title));
        return { data, error: null };
    } catch (error) {
